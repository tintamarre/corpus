<template>
  <div class="row">
    ...
    <div class="col-sm-12 col-md-8 col-lg-8">
      ...
      <button @click="revertText" v-if="expandedText" class="btn btn-sm btn-link">
        <fa icon="minus"></fa> {{ __('app.reduce') }}
      </button>


      <div class="text-muted small text-right">
        <em>in</em> <a :href="snip.text.links.self">{{ snip.text.name }}</a> {{ __('app.tagged_by') }} <strong>{{ snip.author }}</strong>
      </div>
...
export default {
  ...
  computed: {
    wrapContent() {
      return '<span class="text-muted">...</span> ' + this.snip.segment_content.substring(this.snip.snippet_start - this.offset,this.snip.snippet_start) +
      "<span class='bg-warning'>" +
      this.snip.segment_content.substring(this.snip.snippet_start, this.snip.snippet_end) +
      "</span>" +
      this.snip.segment_content.substring(this.snip.snippet_end, this.snip.snippet_end + this.offset) + ' <span class="text-muted">...</span>';
    },
  },
  methods: {
    expandText() {
